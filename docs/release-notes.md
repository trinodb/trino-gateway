...
* Support in-memory caching of backend metadata retrieved from the Trino Gateway
  database with the `databaseCacheTTL` setting.
  ([#783](https://github.com/trinodb/trino-gateway/pull/783))
* Add `queryHistoryEnabled` setting to deactivate query history recording in the
  Trino Gateway database. 
  ([#800](https://github.com/trinodb/trino-gateway/pull/800))

**UI**

* Add drop down widget to allow setting of the time zone for display.
