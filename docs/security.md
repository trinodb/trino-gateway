...
> **When none of the listed methods is configured:** the login page falls back to
> the methods that *are* configured, so it never renders empty and the gateway
> still boots. For example, `defaultType: "oauth"` with only a `form` block
> configured shows the form login (and a warning is logged). Startup fails fast
> only when a `defaultType` entry is unknown/misspelled, when `defaultType` is
> empty, or when no `oauth`/`form` block is configured at all.
