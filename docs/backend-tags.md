...
!!! warning "Manual migrations"
    If `runMigrationsEnabled` is `false`, apply
    `V5__add_tags_to_gateway_backend.sql` before upgrading. All backend
    create and update requests fail until the `tags` column exists, even when
    no tags are used.
...
!!! note "Commas are not allowed in tag values"
    A tag value cannot contain a `,` character — the gateway rejects the
    request with an error response and the backend is left unchanged. If you
    need to express multiple values for the same key, split them into separate
    tags rather than combining them with a comma. For example, instead of
