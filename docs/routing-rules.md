...
references a view, then that view will not be expanded, and tables referenced by the view will not be recognized.
A routing rule can call the following methods on the `trinoQueryProperties` object:   
* `boolean isQuery()`: is the request a valid SQL query. Note that this will return false for invalid SQL.
...
`TrinoQueryProperties` is only available if enabled by configuring `requestAnalyzerConfig.isAnalyzeRequest = True`.
Support for [V2 style request structure](https://github.com/trinodb/trino/wiki/Trino-v2-client-protocol#submit-a-query) 
is enabled by setting `requestAnalyzerConfig.isClientsUseV2Format = True`. By default, the max body size is 
1,000,000 characters, this can be modified by configuring `requestAnalyzerConfig.maxBodySize`.
