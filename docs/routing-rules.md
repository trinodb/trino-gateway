...
route requests. Use the `defaultRoutingGroup` setting to specify a fallback routing group.
Otherwise, the fallback is the built-in `adhoc`.
...
* If errors is not null, then the query is routed to either the configured default routing.
