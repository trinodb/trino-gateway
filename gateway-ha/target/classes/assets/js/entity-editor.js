...
function updateObject() {
    ...
    submitData("/entity?" + $.param(updateParams), jsonVal, "post").done(function (data) {
        $("#submitted_status").css({"display": "block", "background-color": "green"})
        $("#submitted_status").text("SUCCESS")
        setTimeout(() => $("#submitted_status").css({"display": "none"}), 3000)
        renderEntitySelector();
    }).fail(function(data) {
        $("#submitted_status").css({"display": "block", "background-color": "red"})
        $("#submitted_status").text("FAILED")
        setTimeout(() => $("#submitted_status").css({"display": "none"}), 3000)
    })
}
