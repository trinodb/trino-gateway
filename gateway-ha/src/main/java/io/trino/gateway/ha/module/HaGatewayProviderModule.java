        ...
        if (routingRulesConfig.isRulesEngineEnabled()) {
            try {
                return switch (routingRulesConfig.getRulesType()) {
                    ...
                    case API -> {
                        RulesRESTConfiguration rulesRESTConfiguration = routingRulesConfig.getRulesRESTConfiguration();
                        yield RoutingGroupSelector.byRoutingRestfulApi(rulesRESTConfiguration, configuration.getRequestAnalyzerConfig());
                    }
                };
            }
            catch (Exception e) {
