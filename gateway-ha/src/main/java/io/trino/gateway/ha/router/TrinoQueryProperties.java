...
public class TrinoQueryProperties
{
    ...
    public TrinoQueryProperties(HttpServletRequest request, RequestAnalyzerConfig config)
    {
        ...
        defaultCatalog = requireNonNullElse(request.getHeader(TRINO_CATALOG_HEADER_NAME), UNSET);
        defaultSchema = requireNonNullElse(request.getHeader(TRINO_SCHEMA_HEADER_NAME), UNSET);
    ...
    private void processRequestBody(HttpServletRequest request, RequestAnalyzerConfig config)
    {
        try (BufferedReader reader = request.getReader()) {
            ...
            // Note that this does necessarily not match the Resource Group query type. io.trino.util.StatementUtils maps statement
            // types to a smaller list of query types.
            ...
            if (catalogs.isEmpty()) {
                catalogs = tables.stream().map(q -> q.getParts().getFirst()).collect(Collectors.toSet());
            }
            if (schemas.isEmpty()) {
                schemas = tables.stream().map(q -> q.getParts().get(1)).collect(Collectors.toSet());
            }
            if (catalogSchemas.isEmpty()) {
                catalogSchemas = tables.stream()
                        .map(qualifiedName -> String.format("%s.%s", qualifiedName.getParts().getFirst(), qualifiedName.getParts().get(1)))
        ...
        while (headers.hasMoreElements()) {
            ...
            for (String preparedStatement : preparedStatementsArray) {
                ...
                if (nameValue.length != 2) {
                    throw new InputMismatchException(String.format("preparedStatement must be formatted as name=value, but is %s", preparedStatement));
                }
                preparedStatementsMap.put(URLDecoder.decode(nameValue[0], UTF_8), URLDecoder.decode(decodePreparedStatementFromHeader(nameValue[1]), UTF_8));
            }
        }
        return preparedStatementsMap;
    }
        ...
        switch (node) {
            ...
            case RenameSchema s -> {
                ...
                if (s.getSource().getParts().size() == 1) {
                    targetSchema = QualifiedName.of(defaultCatalog, s.getTarget().getValue());
                }
                else {
            ...
            case ShowSchemas s -> catalogBuilder.add(s.getCatalog().map(Identifier::getValue).orElse(defaultCatalog));
            ...
            ImmutableSet.Builder<String> catalogSchemaBuilder)
    {
        if (schemaOptional.isEmpty()) {
            schemaBuilder.add(defaultSchema);
            catalogBuilder.add(defaultCatalog);
            ...
            switch (schema.getParts().size()) {
                case 1:
                    //String schema = schema.getParts().getFirst()
                ...
                case 2:
                    ...
                    catalogSchemaBuilder.add(String.format("%s.%s", schema.getParts().getFirst(), schema.getParts().get(1)));
            }
        }
    }
    ...
    private QualifiedName qualifyName(QualifiedName table)
    {
        ...
        return switch (tableParts.size()) {
            case 1:
                yield QualifiedName.of(defaultCatalog, defaultSchema, tableParts.getFirst());
            case 2:
                yield QualifiedName.of(defaultCatalog, tableParts.getFirst(), tableParts.get(1));
            case 3:
            default:
    ...
    {
        // Based on https://github.com/prestodb/presto/wiki/Presto-v2-client-protocol, without session
