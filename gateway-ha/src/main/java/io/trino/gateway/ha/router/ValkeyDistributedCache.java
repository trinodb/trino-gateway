            ...
            long healthCheckIntervalMs)
    {
        ...
        try {
            if (password != null && !password.isEmpty()) {
                this.pool = new JedisPool(poolConfig, host, port, timeoutMs, password, database);
            }
            else {
                this.pool = new JedisPool(poolConfig, host, port, timeoutMs, null, database);
            }
            ...
            log.error(e, "Failed to initialize Valkey connection pool");
            throw new RuntimeException("Failed to initialize Valkey", e);
        }
    }
