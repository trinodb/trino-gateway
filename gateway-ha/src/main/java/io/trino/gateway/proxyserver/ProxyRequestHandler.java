...
public class ProxyRequestHandler
{
    ...
    private Response handleProxyException(Request request, ProxyException e, RoutingDestination routingDestination)
    {
        if (isExpectedlyUnavailable(routingDestination.clusterName(), gatewayBackendManager, backendStateManager)) {
            log.debug(e, "Proxy request failed, backend is inactive and unreachable: %s %s", request.getMethod(), request.getUri());
            throw new WebApplicationException(
                    Response.status(SERVICE_UNAVAILABLE)
                            .type(TEXT_PLAIN_TYPE)
                            .entity("Backend cluster %s is unreachable, it has been intentionally scaled down.".formatted(routingDestination.clusterName()))
                            .build());
        }
