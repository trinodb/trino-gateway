...
public class TestAuthorizationManager
{
    ...
    public void testCustomDefaultPrivilege()
    {
        ...
        authConfig.setEnableDefaultPrivilege(true);
    ...
    @Test
    public void testDefaultPrivilegeIgnoredWhenDisabled()
    {
        // enableDefaultPrivilege defaults to false, so the configured value is ignored and
        // unmatched users are denied.
        AuthorizationConfiguration authConfig = new AuthorizationConfiguration();
        authConfig.setDefaultPrivilege("USER");
        AuthorizationManager authManager = createAuthorizationManager(authConfig, Map.of());

        assertThat(authManager.getPrivileges("newUser")).isEmpty();
    }
    ...
    @Test
    public void testPresetUserWithEmptyPrivilegesFallsBackToDefault()
    {
        UserConfiguration presetUser = new UserConfiguration("", "password"); // Empty privileges
        AuthorizationManager authManager = new AuthorizationManager(Map.of("emptyPrivUser", presetUser), null, Optional.of("USER"));

        // A preset user with no configured privileges still resolves to the default privilege.
        assertThat(authManager.getPrivileges("emptyPrivUser")).hasValue("USER");
    }

    @Test
    public void testLdapMembershipTakesPrecedenceOverDefault()
    {
        LbLdapClient ldapClient = mock(LbLdapClient.class);
        when(ldapClient.getMemberOf("ldapUser")).thenReturn("ADMIN");
        AuthorizationManager authManager = new AuthorizationManager(Map.of(), ldapClient, Optional.of("USER"));

        assertThat(authManager.getPrivileges("ldapUser")).hasValue("ADMIN");
    }

    @Test
    public void testLdapEmptyResultDeniesEvenWithDefaultPrivilege()
    {
        LbLdapClient ldapClient = mock(LbLdapClient.class);
        when(ldapClient.getMemberOf("ldapUser")).thenReturn(""); // getMemberOf returns "" when the search yields nothing

        AuthorizationManager authManager = new AuthorizationManager(Map.of(), ldapClient, Optional.of("USER"));

        // LDAP is authoritative when configured: an empty result denies access instead of
        // widening it to defaultPrivilege, so a misconfigured search can't silently grant access.
        assertThat(authManager.getPrivileges("ldapUser")).isEmpty();
    }
