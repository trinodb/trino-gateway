...
final class TestLbTokenUtil
{
    ...
    private final String ecPrivateKey = "auth/test_ec_private_key.pem";
    private final String ecPublicKey = "auth/test_ec_public_key.pem";
    ...
    void testEcTokenValidationWithAudiences()
    {
        LbKeyProvider ecKeyProvider = new LbKeyProvider(new SelfSignKeyPairConfiguration(
                requireNonNull(getClass().getClassLoader().getResource(ecPrivateKey)).getFile(),
                requireNonNull(getClass().getClassLoader().getResource(ecPublicKey)).getFile()));
